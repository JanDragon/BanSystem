...
public class MySQL {
    ...
    private void connect() {
        try {
            ...
            ProxyServer.getInstance().getConsole().sendMessage(Data.getPrefix() + " §aEine Verbindung zur MySQL war erfolgreich");
        } catch (Exception e) {
            ProxyServer.getInstance().getConsole().sendMessage(Data.getPrefix() + " §cDie Verbindung zum MySQL-Server ist fehlgeschlagen, da deine Daten leider falsch sind");
            e.printStackTrace();
        }
    }
