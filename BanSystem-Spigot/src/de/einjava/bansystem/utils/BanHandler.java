...
public class BanHandler {
    ...
    public void banPlayer(CommandSender sender, String reason) {
        ...
        if (player != null) {
            ...
            player.kickPlayer(getBanScreen());
        } else {
            for (UUID uuid : Data.notify) {
                Player p = Bukkit.getPlayer(uuid);
                if (p.hasPermission(Config.getConfiguration().getString("Command.Notify.Permission"))) {
        ...
        Bukkit.getPluginManager().callEvent(new PlayerBanEvent(name, sender.getName(), getExactReason(reason), banHandler.getDuration()));
    }
    ...
    public void unBanPlayer(CommandSender sender) {
        ...
        banHandler.setID(0);
        Bukkit.getPluginManager().callEvent(new PlayerUnBanEvent(name, sender.getName()));
    }

    public void reduceBan(String mod, int days) {
        Player moderator = Bukkit.getPlayer(mod);
        BanHandler banHandler = new BanHandler(name);
        if (!banHandler.isBanned()) {
        ...
        Bukkit.getPluginManager().callEvent(new BanReduceEvent(name, moderator.getName(), seconds));
    }
